...
<div class="container">
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="card p-4">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-2">
                        <input type="text" name="reference_line" id="reference-line" class="form-control" maxlength="20" placeholder="Ligne" value="{{ old('reference_line', $refLine) }}">
                    </div>
                    <div class="col-2">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="codebar" class="form-control" value="{{ $product->codebar }}">
            </div>

            <div class="col-md-6">
                <label class="form-label">Designation</label>
                <input type="text" name="marque" class="form-control" value="{{ $product->marque }}" required>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="category_id" class="form-select">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ (int)$product->category_id === (int)$category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
                <input type="hidden" name="categorie" value="{{ $product->categorie ?: 'Produit' }}">
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="etat" class="form-select" required>
                    ...
                    <option value="Occasion" {{ $product->etat === 'Occasion' ? 'selected' : '' }}>Occasion</option>
                </select>
            </div>

            <div class="col-md-2">
                <label class="form-label">TVA %</label>
                <input type="number" step="0.01" min="0" max="100" name="tva_rate" id="tva-rate" class="form-control" value="{{ old('tva_rate', $product->tva_rate ?? 20) }}">
            </div>

            <div class="col-md-3">
                <label class="form-label">Prix achat HT</label>
                <input type="number" step="0.01" min="0" name="prix_achat_ht" id="prix-achat-ht" class="form-control" value="{{ old('prix_achat_ht', $product->prix_achat_ht ?? $product->prix_achat) }}" required>
            </div>

            <div class="col-md-3">
                <label class="form-label">Prix achat TTC</label>
                <input type="number" step="0.01" min="0" name="prix_achat_ttc" id="prix-achat-ttc" class="form-control" value="{{ old('prix_achat_ttc', $product->prix_achat_ttc ?? $product->prix_achat) }}" required>
            </div>

            <div class="col-md-2">
                <label class="form-label">Prix vente HT</label>
                <input type="number" step="0.01" min="0" name="prix_vente_ht" id="prix-vente-ht" class="form-control" value="{{ old('prix_vente_ht', $product->prix_vente_ht ?? $product->prix_vente) }}" required>
            </div>

            <div class="col-md-2">
                <label class="form-label">Prix vente TTC</label>
                <input type="number" step="0.01" min="0" name="prix_vente_ttc" id="prix-vente-ttc" class="form-control" value="{{ old('prix_vente_ttc', $product->prix_vente_ttc ?? $product->prix_vente) }}" required>
                <small class="text-muted">Utilise en caisse</small>
            </div>

            <div class="col-md-4">
                <label class="form-label">Stock total</label>
                <input type="number" id="stock-input" name="quantite" class="form-control" value="{{ $product->quantite }}">
            </div>

            <div class="col-12">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" role="switch" id="hasVariantsSwitch" name="has_variants" value="1" {{ $product->has_variants ? 'checked' : '' }}>
            ...
            <div class="col-md-6">
                ...
                <select name="fournisseur_id" class="form-select">
                    ...
                    @foreach($fournisseurs as $f)
                        <option value="{{ $f->id }}" {{ (string)$product->fournisseur_id === (string)$f->id ? 'selected' : '' }}>{{ $f->nom }}{{ $f->societe ? ' - '.$f->societe : '' }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-6">
                ...
                @if($product->image)
                    <div class="mt-2">
                        <img src="{{ asset($product->image) }}" alt="image" style="max-height:90px; border-radius:8px; border:1px solid #ddd;">
                    </div>
                @endif
            </div>

            <div class="col-12" id="variant-section" style="{{ $product->has_variants ? '' : 'display:none;' }}">
...
const existingVariants = @json($product->variants->map(function($v){ return ['size' => $v->size, 'color_id' => $v->color_id, 'quantity' => $v->quantity]; }));
...
function addVariantRow(size = '', colorId = '', quantity = 0) {
    ...
    row.innerHTML = `
        <div class="col-md-4">
            <input type="text" name="variant_size[]" class="form-control" placeholder="Taille" value="${size || ''}">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-3">
            <input type="number" min="0" name="variant_quantity[]" class="form-control variant-qty" value="${quantity || 0}">
        </div>
...
if (existingVariants.length) {
    existingVariants.forEach((variant) => addVariantRow(variant.size, variant.color_id, variant.quantity));
} else if (document.getElementById('hasVariantsSwitch').checked) {
    addVariantRow();
}
